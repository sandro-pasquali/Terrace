...
module.exports  = function(options) {
    ...
    var global = $.document() ? window : this;

//////////////////////////////////////////////////////////////////////////////////////////
//																						//
//	When this lib is updated, cut and paste between this and the similar tail marker.	//
//	stripping out the loading context, handling global as above.						//
//																						//
//	No license given, assuming MIT or similar.											//
//																						//
...
$.addKit("receiver", {
	...
	listen	: function(eventId, open) {
		var openTime;

		if(!eventId) {
			return $.error("Attempt to bind receiver without an @id");
		}

		//	There is only one event source object ever in play.
		//
		if(source) {
			open && open(openTime);
			return;
		...
		source = new EventSource("/bindReceiver/" + eventId);
		source.addEventListener('open', function() {
			openTime = new Date();
			open && open(openTime);
			$.publish("%receiver:open", openTime);
		}, false);

		source.addEventListener('message', function(messageObject) {
			var id 		= messageObject.lastEventId;
			var mess	= messageObject.data;

			//	We expect JSON. If not, create.
			//
			try {
				mess = JSON.parse(mess);
			} catch(e) {
				mess = JSON.parse('{"data":"' + mess.replace('"','\"') + '"}');
			}
			...
			$.publish("%receiver:" + id, mess);

		}, false);

		source.addEventListener('error', function(err) {
			$.publish("%receiver:error", err);
		}, false);
	},
