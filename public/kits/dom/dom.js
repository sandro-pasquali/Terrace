...
//	TODO: run Dom.configure below using a translation of #options
//
...
//
Dom = typeof $.document() ? ((NW.Dom = { }) && (global = window)) : this,

// processing context & root element
doc = (window || this)["document"],
root = doc.documentElement || this,
...
var toFunc = function(a) {
	return $.is(Function, a) ? a : function() { return a; };
};
...
//	@see		#dom
//
var selC 	= function(sel, coll) {

	var ns		= $.dom.$get("selectorCache"),
		i 		= 0,
		f 		= [];
...
$.addKit("dom", {
	...
	//	Find all elements matching selector in context. If context is an array will
	//	return an array of all found, in order.
	//
	//	Always returns an array, which may be empty.
	//
	find 	: function(selector, context) {
		context = context || this.$;
		context	= !$.is(Array, context) ? [context] : context;
		...
		while(i < len) {
			t = Dom.select(selector, context[i]) || [];
			res = res.concat($.is(Array, t) ? t : [t]);
			i++;
		}
...
//	Set up a namespace for cached selectors.
//	@see	#after
//	@see	#before
//
$.dom.set('selectorCache', {});
