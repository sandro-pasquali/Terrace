...
CSS_FILES 	= $(shell find public/bits -not -name "*.min.css" -name '*.css')
JS_FILES 	= $(shell find public/bits -not -name "*.min.js" -name '*.js')
...
describe('Person', function() {\
	it('should say hello', function() {\
		var person 	= new global.terraceTesting.Person;\
		person.sayHello('Sandro').should.equal('Hello, Sandro!');\
	});\
});"


SAMPLE_SRC = "global.terraceTesting = {};\
global.terraceTesting.Person = function() {\
...
#	and then pass execution to > node bin/build which handles the rest.
#
...
.PHONY: update install test help clean minify minify-css minify-js minify-clean 
